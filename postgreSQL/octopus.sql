...
CREATE TABLE commit(                                                    --
    ...
    developer_id INT,                   /* worker who committed */      --
...
-- adding foreign key constraint seperately because of 
-- cyclic forign key references
...
CREATE OR REPLACE FUNCTION login_user(user_name VARCHAR, password VARCHAR)
RETURNS TABLE (developer_id INT)
AS $$
BEGIN
IF NOT EXISTS
...
THEN
        RETURN QUERY SELECT -1 as developer_id;
        RETURN;
END IF;
    UPDATE developer
    SET last_visit = TO_CHAR(current_timestamp, 'YYYY-MM-DD HH:MI:SS AM')::VARCHAR
    where developer.user_name = login_user.user_name;

RETURN  QUERY SELECT (SELECT developer.developer_id FROM developer WHERE developer.user_name = login_user.user_name and developer.encrypted_password = (crypt(login_user.password, '$2024$DBMS$fixedsalt'))) as developer_id;
    RETURN;
END;
$$ LANGUAGE plpgsql;
...
if (create_user.user_name in (select developer.user_name from developer)) then
    ...
    RETURN QUERY SELECT false AS is_created, CAST('User name already exists!!' AS VARCHAR) AS msg;
    RETURN;
end if;

/* checking for uniqueness of email */
IF (create_user.email in (select developer.email from developer))
THEN
        RETURN QUERY SELECT false AS is_created, CAST('Email id is not unique' AS VARCHAR) AS msg;
        RETURN;
END IF;
...
RETURN QUERY SELECT true AS is_created, CAST('Account created' AS VARCHAR) AS msg;
RETURN;
END;
$$ LANGUAGE plpgsql;
...
$$ LANGUAGE plpgsql;
--------------------------------------------------------------------------
DROP FUNCTION IF EXISTS get_all_repos;
CREATE OR REPLACE FUNCTION  get_all_repos(current_user_name VARCHAR, search_user_name VARCHAR)                            
RETURNS TABLE(type VARCHAR, id INT, name VARCHAR, parent_id INT)
AS $$
BEGIN
    RETURN QUERY
    SELECT CAST('REPO' AS VARCHAR), r.repository_id, r.repository_name, r.parent_id
    FROM developer d, repository r
    WHERE d.user_name = search_user_name and r.owner_id = d.developer_id and can_view(current_user_name, r.repository_id);

    RETURN QUERY
    SELECT CAST('FILE' AS VARCHAR), f.file_id, f.file_name, f.parent_repository_id
    FROM file f
    WHERE f.parent_repository_id in 
...
SELECT * FROM get_all_repos('_sandeep_', '_sandeep_');
...
-----------------------------------------*      test area start      *-----------------------------------------
SELECT * from create_user('_sandeep_', 'sandeep reddy', '112101011@smail.iitpkd.ac.in', '1111');
SELECT * FROM developer;
SELECT * FROM repository;
SELECT * FROM branch;
SELECT * FROM login_user('_sandeep_', '2122');
...
SELECT create_user('_manish_', 'Manish M H', '112101002@smail.iitpkd.ac.in', '3333');
SELECT * FROM developer;
SELECT * FROM repository;
SELECT * FROM file;
select * from access;
